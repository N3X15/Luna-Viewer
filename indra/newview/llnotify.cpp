...
bool gNotifyBoxesShouldSTFU=FALSE;
bool gFuckNotifyBoxes=FALSE;
	  ...
	  mDefaultOption(0)
{
	...
	else
	{
		...
		// FUUUUUUUCK
		if	(sNotifyBoxCount > SHL_SENSIBLE_NOTIFY_BOX_COUNT-1 || gNotifyBoxesShouldSTFU == TRUE)
		{
			gFuckNotifyBoxes=TRUE;
			mIsTip=1;
		}
	}
}
...
void LLNotifyBox::close()
{
	...
	if(sNotifyBoxCount<SHL_SENSIBLE_NOTIFY_BOX_COUNT)
	{
		gFuckNotifyBoxes=FALSE;
	}
	if(sNotifyBoxCount==0)
	{
		gNotifyBoxesShouldSTFU=FALSE;
	}
	die();
	if(!isTipTmp)
	{
