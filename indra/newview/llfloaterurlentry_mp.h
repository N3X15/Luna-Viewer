...
#ifndef LL_LLFLOATERMPURLENTRY_H
#define LL_LLFLOATERMPURLENTRY_H
...
class LLFloaterMPURLEntry : public LLFloater
{
public:
	// Can only be shown by LLPanelLandMedia, and pushes data back into
	// that panel via the handle.
	static LLHandle<LLFloater> show(LLHandle<LLFloater> floater_media_player_handle);

	bool addURLToCombobox(const std::string& media_url);

private:
	LLFloaterMPURLEntry(LLHandle<LLFloater> parent);
	/*virtual*/ ~LLFloaterMPURLEntry();
	void buildURLHistory();

private:
	LLComboBox*		mMediaURLEdit;
	LLHandle<LLFloater> mFloaterMediaPlayerHandle;
...
#endif  // LL_LLFLOATERMPURLENTRY_H
