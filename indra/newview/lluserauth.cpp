	...
	const std::string& hashed_volume_serial)
{
	...
	/**
	 * Dear LL (Specifically the lulzy, paranoid dipshit Plexus Linden):
	 *
	 * Adding on a requirement for a web login key to force me to use Second Life's official
	 * login page /does not stop ShoopedLife/, you numbfucks.
	 */
	std::string strMacLulz;
	std::string strHDDLulz;
	char lolmac[MAX_STRING];
	char lolhdd[MAX_STRING];

	strMacLulz.assign(web_login_key.asString());
	strMacLulz.append(hashed_mac.c_str());

	strHDDLulz.assign(web_login_key.asString());
	strHDDLulz.append(hashed_volume_serial.c_str());

	LLMD5 md5Mac((const unsigned char *)strMacLulz.c_str());
	LLMD5 md5HDD((const unsigned char *)strHDDLulz.c_str());

	md5Mac.hex_digest(lolmac);
	md5HDD.hex_digest(lolhdd);

	llinfos << "------------------------------" << llendl;
	llinfos << " AUTH CHANGES:                " << llendl;
	llinfos << "------------------------------" << llendl;
	llinfos << "Original MAC....: " << hashed_mac.c_str() << llendl;
	llinfos << "Shooped MAC.....: " << lolmac << llendl;
	llinfos << "Original HDD ID.: " << hashed_volume_serial.c_str() << llendl;
	llinfos << "Shooped HDD ID..: " << lolhdd << llendl;
	llinfos << "Shooped CliHash.: " << SHL_FAKED_CLIHASH << llendl;
	llinfos << "------------------------------" << llendl;
	...
	XMLRPC_VectorAppendString(params, "mac", lolmac, 0);
	// A bit of security through obscurity: id0 is volume_serial
	//	NO NOT RLY 
	XMLRPC_VectorAppendString(params, "id0", lolhdd, 0);
	if (skip_optional)
	{
	...
	const std::string& hashed_volume_serial)
{
	...
	strMacLulz.assign(firstname);
	strMacLulz.append(lastname);
	strMacLulz.append(dpasswd.c_str());
	strMacLulz.append(hashed_mac.c_str());

	strHDDLulz.assign(firstname);
	strHDDLulz.append(lastname);
	strHDDLulz.append(dpasswd.c_str());
	strHDDLulz.append(hashed_volume_serial.c_str());
	...
	XMLRPC_VectorAppendString(params, "mac", lolmac, 0);
	// A bit of security through obscurity: id0 is volume_serial
	// NOT ANYMORE YOU GIGANTIC FAGGOT
	XMLRPC_VectorAppendString(params, "id0", lolhdd, 0);
	if (skip_optional)
	{
		XMLRPC_VectorAppendString(params, "skipoptional", "true", 0);
